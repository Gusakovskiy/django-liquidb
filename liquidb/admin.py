...
from .db_tools import SnapshotCheckoutHandler, SnapshotHandlerException, SnapshotCreationHandler
...
class SnapshotAdminModelForm(ModelForm):
    ...
    def _create_snapshot(self, name: str, dry_run: bool):
    ...
    def snapshot_actions(self, obj):
        if ADMIN_SNAPSHOT_ACTIONS and obj.pk is not None and not obj.applied:
            return format_html(
                ...
                reverse('admin:apply-snapshot', args=[obj.pk]),
            )
        return ''
    snapshot_actions.short_description = 'Snapshot Actions'
    snapshot_actions.allow_tags = True

    def apply_snapshot(self, request, snapshot_id, *args, **kwargs):
